...
class ANOVA(StatFormatter):
    ...
    def __init__(self,
        ...
        if nan_policy == "propogate":
            ...
            self.cor_data = self._correct_nans()
        ...
        if way == 1:
            self.output = [{"id"  : ["ANOVA"],
                           ...
                           "alpha"       : [alpha]}]
            self.output[0]["sig"] = [self.output[0]["pvalue"] < self.output[0]["alpha"]]
            self.fill_values()
        else:
...
class TukeyHSD(ANOVA):
    ...
    def __init__(self,
        ...
        if self.output[0]["pvalue"][0] >= alpha and not override:
            ...
            self.output.append({"Post hoc tests" : ["Are not permitted."]})
        # Otherwise
        else:
            # Save the results 
    ...
    def tukey_hsd(self,
        ...
        for pair in paired:
            ...
            tukeyout["pvalue"].append(float(1 - q.cdf(self.q_statistic(pair[0][1],
                                                                 pair[1][1],
                                                                 msw), k, df)))
            tukeyout["sig"].append(tukeyout["pvalue"][-1] < alpha)
        if self.override:

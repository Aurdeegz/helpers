...
def _make_transmap():
    ...
    for i in range(16,126):
        btw.append([(i*2)/255, 1,1,1])
        ptw.append([1,(i*2)/255,1,1])
...
def modified_boxplot(data_cols, 
            group_labs, 
            colours = ["grey" for _ in range(30)],
            ylabel = "ylabel",
            ylims = None,
            fontdict = dict(fontfamily = "sans-serif",
                            ...
                            fontweight = "bold"),
                    savefig = None):
    # For boxplots, you need to make the box and then adjust the aspects of it
    # so first, I guess we make the boxes 
    fig, ax = plt.subplots(1,1,figsize=(len(data_cols),len(data_cols)))
    bp = ax.boxplot(data_cols,
                    notch=True,
                    showcaps=False,
                    patch_artist = True,
                    positions = [i/2 for i in range(len(data_cols))],
                    widths = [0.4 for i in range(len(data_cols))],
                    sym = ".",
                    flierprops = dict(marker=".",
                                      ...
                                      markersize=5,
                                      alpha = 0.5) )
    ax.set_xticks([i/2 for i in range(len(data_cols))])
    ax.set_xticklabels(group_labs, rotation = 90, fontsize = 10, **fontdict)
    update_ticks(ax, which="y")
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ax.set_ylabel(ylabel, fontsize = 12, **fontdict)
    if ylims:
        ax.set_ylim(ylims[0], ylims[1])
    else:
        ylims = list(ax.get_ylim())
    bp_meds = [item.get_ydata()[0] for item in bp["medians"]]
    for i in range(len(data_cols)):
        ax.text(i/2, ylims[1], f"{bp_meds[i]:.2f}", va = "center", 
                        ha = "center", fontsize = 8, **fontdict)
    for i in range(len(bp["boxes"])):
        bp["boxes"][i].set_facecolor(colours[i])
        bp["medians"][i].set(color="black")
        bp["fliers"][i].set(color=colours[i])
    if savefig != None:
        # Assume this is a string with a file extension
        plt.savefig(savefig)
    return bp

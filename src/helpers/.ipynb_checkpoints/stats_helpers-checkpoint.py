...
def storey(pvals, 
    ...
    lambdas = np.arange(0.01,0.97,0.02)
    ...
    # Otherwise, determine an appriopriate value.
        ...
        if hat_pi0 > 1:
            ...
            print(f"Fitting the cubic spline failed (hat_pi0 = {hat_pi0}), resorting to BH")
            hat_pi0 = 1
...
class StatFormatter():
    ...
    def write_output(self, filename = "output", file_type = "csv", writestyle = "w"):
        ...
        if file_type.lower() in ["xls", "xlsx"]:
            ...
            for line_list in organised:
                ...
                outlines.append("\n\n")
            gh.write_outfile(outlines, output_file, writestyle = writestyle)
...
class TTest(StatFormatter):
    ...
    def t_degree_freedom(self):
        ...
        if self.tt in ["s", "student"]:
            ...
            #print(f"numer = {numer:.5f} | denom = {denom:.5f}")
            return numer / denom
...
class TukeyHSD(ANOVA):
    ...
    def __init__(self,
        ...
        if self.output[0]["pvalue"][0] >= alpha and not override:
            ...
            self.output.append(self.tukey_hsd(*self.cor_data, alpha = alpha, labels = labels, no_comp = no_comp))

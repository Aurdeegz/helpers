...
def licor_correction(exp_signal, control_signal):
    ...
    if any([item!=item for item in control_signal]):
        pseudo_control = [item for item in control_signal if item == item]
    else:
        pseudo_control = [item for item in control_signal]
    # Make the correction factors using the max signal
    corr_facts = [signal / max(pseudo_control) for signal in control_signal]

...
def _volcano_cutoffs(ax, sig_cutoffs, fc_cutoff, sig_cutoff_strs,
    ...
    """
    print(f"show_siglabel = {show_siglabel}")
...
def remove_nanpairs(list1, list2):
    paired = list(zip(list1, list2))
    paired = [row for row in paired if all([item == item for item in row])]
...
def volcano(sig_list, foldchanges, fc_transform = "none", sig_transform = "log10", sig_comp = "<",
    ...
    print(f"xlim = {xlim}")
    # Remove any nans first because it's annoying
    sig_list, foldchanges = remove_nanpairs(sig_list, foldchanges)
    if axes == None:
    ...
    if title != None:
        print(f"title = {title}")
        axes.text(0.5,1.04, title, fontsize = 14, bbox=dict(facecolor='none', edgecolor='black'), 
        ha = 'center', transform = axes.transAxes, **fontdict) # transAxes lets you plot in axes coords
        #axes.set_title(title, fontsize = 14, bbox=dict(facecolor='none', edgecolor='black'), **fontdict),pad=10,
        #               **fontdict)
    if xlabel != None:
        axes.set_xlabel(xlabel, fontsize = 12, **fontdict)
    if ylabel != None:
        axes.text(-0.15, 0.5, ylabel, rotation = 90, fontsize = 12, ha = "center", 
        va = "center", transform = axes.transAxes, **fontdict)
    ...
    if fc_transform == "log2":
        if not x_axis_turnoff:
            if fc_label_transform == "none":
                new_tickstrs = [int(item) for item in list(axes.get_xticks()) if item == int(item)]
                axes.set_xticks(new_tickstrs)
            else:
...
def volcano_array(sig_list_array,foldchange_array,
                left_labels = [fr"$\log_{{10}}(q)$" for _ in range(20)],
                   right_labels = ["PENIS" for _ in range(20)],
                   bottom_labels = ["Fold-change" for _ in range(20)], 
                   top_labels = ["TOOOOP" for _ in range(20)], 
                   colours = None,
                   save_arr = None, 
                  hush = True,
                   **volcano_kwargs):
    ...
    figure, ax_arr = plt.subplots(nrows = len(sig_list_array), ncols = len(sig_list_array[0]), figsize = (len(sig_list_array[0])*4,len(sig_list_array)*4))
    if colours == None:
        colours = [[["grey" for _ in range(len(volcano_kwargs["sig_cutoffs"]))] 
                 for i in range(len(sig_list_array[0]))] for j in range(len(sig_list_array)) ]
    #ylim, xlim = global_lims(sig_list_array, foldchange_array)
    if len(sig_list_array) == 1:
        # Then there is only one row, so we have to handle this separately
        for col in range(len(sig_list_array[0])):
            ...
            if col == 0:
                volcano_kwargs["ylabel"] = left_labels[0]
                volcano_kwargs["y_axis_turnoff"] = False
                volcano_kwargs["show_spines"]["left"] = True
                ax_arr[col] = volcano(sig_list_array[0][col], foldchange_array[0][col], 
                                      axes = ax_arr[col], colours = colours[0][col],
                                           **volcano_kwargs)
            elif col == len(sig_list_array[0])-1:
                volcano_kwargs["ylabel_right"] = right_labels[0]
                volcano_kwargs["ylabel"] = None
                volcano_kwargs["y_axis_turnoff"] = True
                volcano_kwargs["show_spines"]["right"] = True
                volcano_kwargs["show_siglabel"] = True
                volcano_kwargs["show_fclabel"] = True
                ax_arr[col] = volcano(sig_list_array[0][col], foldchange_array[0][col], axes = ax_arr[col],
                                       colours = colours[0][col],
                                           **volcano_kwargs)
            else:
                volcano_kwargs["y_axis_turnoff"] = True
                ax_arr[col] = volcano(sig_list_array[0][col], foldchange_array[0][col], axes = ax_arr[col],
                                            colours = colours[0][col],
                                           **volcano_kwargs)
    elif len(sig_list_array[0]) == 1:
        # Then there is only one column, so we have to handle this separately
        for row in range(len(sig_list_array)):
            ...
            volcano_kwargs["show_spines"] = {"top" : True,
                                             ...
                                             "right" : True}
            volcano_kwargs["x_axis_turnoff"] = False
            # If you're at the first row, you want to turn the x axis off
            if row == 0:
                volcano_kwargs["show_fclabel"] = True
                volcano_kwargs["ylabel"] = left_labels[row]
                volcano_kwargs["title"] = top_labels[row]
                volcano_kwargs["x_axis_turnoff"] = True
                ax_arr[row] = volcano(sig_list_array[row][0], foldchange_array[row][0], 
                                      axes = ax_arr[row], colours = colours[row][0],
                                           **volcano_kwargs)
            # This is the last row, so turn the x axis back on
            elif row == len(sig_list_array)-1:
                volcano_kwargs["ylabel"] = left_labels[row]
                volcano_kwargs["xlabel"] = bottom_labels[row]
                volcano_kwargs["show_spines"]["bottom"] = True
                volcano_kwargs["show_spines"]["top"] = False
                volcano_kwargs["title"] = None
                volcano_kwargs["show_siglabel"] = True
                volcano_kwargs["show_fclabel"] = False
                ax_arr[row] = volcano(sig_list_array[row][0], foldchange_array[row][0], axes = ax_arr[row],
                                       colours = colours[row][0],
                                           **volcano_kwargs)
            else:
                volcano_kwargs["ylabel"] = left_labels[row]
                volcano_kwargs["x_axis_turnoff"] = True
                volcano_kwargs["show_spines"]["bottom"] = False
                volcano_kwargs["show_spines"]["top"] = False
                ax_arr[row] = volcano(sig_list_array[row][0], foldchange_array[row][0], axes = ax_arr[row],
                                            colours = colours[row][0],
                                           **volcano_kwargs)
    ...
    #                    volcano_kwargs["x_axis_turnoff"] = False
                    ...
                    ax_arr[row][col] = volcano(sig_list_array[row][col], foldchange_array[row][col], axes = ax_arr[row][col],
                                                colours = colours[row][col], **volcano_kwargs)
